...
export const useSilenceStore = defineStore('silence', () => {
  ...
  async function cutSilenceToNewTrack(): Promise<Track | null> {
    if (!audioStore.currentFile) {
      cutError.value = 'No audio file loaded';
      return null;
    }
    ...
    const sourceTrack = tracksStore.selectedTrack || tracksStore.mainTrack;
    if (!sourceTrack) {
    ...
    try {
      ...
      // Create a new track for the cut audio
      console.log('[CutSilence] Creating new track...');
      const cutTrack = createCutTrack(sourceTrack, cutAudioEntry);
  ...
  function createCutTrack(sourceTrack: Track, cutAudio: CutAudioData): Track {
    const cutName = `No Silence - ${sourceTrack.name}`;

    const cutTrack: Track = {
      id: generateId(),
      name: cutName,
      audioId: sourceTrack.audioId,
      type: 'clip',
      start: 0,
      end: cutAudio.duration,
      trackStart: 0,
      muted: false,
      solo: false,
      volume: 1,
    ...
    // Add to tracks store
    tracksStore.addTrack(cutTrack);

    // Select and solo the new cut track
    tracksStore.selectTrack(cutTrack.id);
    tracksStore.setTrackSolo(cutTrack.id, true);
