...
function cloneTrack(track: Track): Track {
  return {
    ...
    channelLanes: track.channelLanes ? track.channelLanes.map(lane => ({
      ...lane,
      clips: lane.clips.map(cloneClip),
      volumeEnvelope: lane.volumeEnvelope ? lane.volumeEnvelope.map(p => ({ ...p })) : undefined,
    })) : undefined,
  };
}
